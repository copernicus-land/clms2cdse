...
# ---------------------------------------------------------------------
def create_cog(input_tiff_path, output_cog_path, resampling):
    """Convert to COG following SynergiseFlow.py semantics.

    1️⃣ Clone the original TIFF
    2️⃣ Apply TIFFSET commands per band (unset 320, set 262=1)
    3️⃣ Convert to COG using Python GDAL (COMPRESS=ZSTD)
    4️⃣ Delete temporary cloned TIFF
    """
    print(f"\n📂 Processing: {os.path.basename(input_tiff_path)}")

    # 1) Clone original TIFF into the destination folder
    out_dir = os.path.dirname(output_cog_path)
    stem, _ = os.path.splitext(os.path.basename(output_cog_path))
    cloned_path = os.path.join(out_dir, f"{stem}_cloned.tif")

    print("→ Cloning original TIFF...")
    try:
        os.makedirs(out_dir, exist_ok=True)
        shutil.copy2(input_tiff_path, cloned_path)
    except Exception as e:
        raise RuntimeError(f"Failed to clone TIFF: {e}")

    # 2) Determine band count and run tiffset adjustments per band
    print("→ Applying TIFFSET commands...")
    ds = gdal.Open(cloned_path)
    if ds is None:
        try:
            os.remove(cloned_path)
        except Exception:
            pass
        raise RuntimeError("Could not open cloned TIFF to determine band count.")
    band_count = ds.RasterCount
    ds = None
    print(f"    → TIFF has {band_count} band(s)")

    for i in range(band_count):
        if not run_command([TIFFSET, "-d", str(i), "-u", "320", cloned_path]):
            try:
                os.remove(cloned_path)
            except Exception:
                pass
            raise RuntimeError("tiffset unset tag 320 failed")
    for i in range(band_count):
        if not run_command([TIFFSET, "-d", str(i), "-s", "262", "1", cloned_path]):
            try:
                os.remove(cloned_path)
            except Exception:
                pass
            raise RuntimeError("tiffset set tag 262 failed")

    # 3) Convert to COG (Python GDAL)...
    print("→ Converting to COG (Python GDAL)...")
    gdal.Translate(
        destName=output_cog_path,
        srcDS=cloned_path,
        format="COG",
        creationOptions=["COMPRESS=ZSTD"]
    )

    # 4) Delete the cloned TIFF
    print("→ Deleting temporary cloned TIFF...")
    try:
        os.remove(cloned_path)
    except Exception:
        pass

    print(f"✅ Finished: {os.path.basename(output_cog_path)}")
...
def find_legends_source(legends_folder: str | None, tif_path: str) -> str | None:
    ...
    if product and range_digits:
        ...
        # Exact match first
        if key in subdirs:
            chosen_src = os.path.join(legends_folder, key)

    # Second: product only (e.g., CPBSA)
    if not chosen_src and product:
        if product in subdirs:
            chosen_src = os.path.join(legends_folder, product)
        else:
            prefixed = sorted([d for d in subdirs if d.startswith(product)])
            if prefixed:
                chosen_src = os.path.join(legends_folder, prefixed[0])

    return chosen_src if chosen_src and os.path.isdir(chosen_src) else None


def copy_legends_filtered(src_dir: str, dest_dir: str, year: str | None) -> bool:
    """Copy legends files selectively by year.

    Rule:
      - Inspect files in src_dir (top-level only). For each filename, find the 3rd underscore.
        If present, take the next 4 characters; if they equal the 4-digit year, it's a match.
      - If at least one match is found, copy only matching files; otherwise copy all files.
    """
    try:
    ...
    if year and len(year) == 4 and year.isdigit():
        for name in entries:
            parts = name.split('_')
            if len(parts) >= 4:
                after_third = '_'.join(parts[3:])
                if len(after_third) >= 4 and after_third[:4] == year:
                    matches.append(name)
...
def process_and_report(input_root, output_folder, resampling, report_dir: str, log_path: str, legends_folder: str | None = None):
    ...
    def flush_product_rows(prod: str | None, rows_to_write: list[dict]):
        ...
        print(f"📊 Report saved at: {excel_path}")
    ...
    for idx, in_file in enumerate(valid_files, start=1):
        ...
        if DO_GENERATE_COGS:
            try:
                create_cog(in_file, out_file, resampling=resampling)
                cog_status = "CREATED"
            except Exception as e:
                cog_status = "ERROR"
        ...
        if parent_dir != last_parent_dir:
            last_legends_src = find_legends_source(legends_folder, in_file)
        ...
        if last_legends_src and os.path.isdir(last_legends_src):
            ...
            if os.path.exists(dest):
                ...
                copied = copy_legends_filtered(last_legends_src, dest, last_year)
...
input_folder = r"M:\raw_data\Crops\CropTypes\CropTypesConfidenceLayers"
legends_folder = r"M:\raw_data\Legends\Combined"
output_folder = r"M:\delivery_data\reingestion-cfg25\Crops\CropTypes\CropTypesConfidenceLayers"
log_path = r"M:\delivery_data\reingestion-cfg25_logs\process_log_CTYCL.csv"
report_dir = r"M:\delivery_data\reingestion-cfg25_logs"
resampling = "MODE"
